...
public class CameraOperator : MonoBehaviour {
    ...
    //vars de controlo do select através da câmera
    public Texture2D selectionHighlight = null;
    public static Rect selection = new Rect(0,0,0,0);
    private Vector3 startClick = -Vector3.one;

    //vars para controlo dos movimentos dos objectos
    ...
    {
        //determina se o o butão do rato(esquerdo = 0) está pressionado, se estiver executa o 1 if
        if (Input.GetMouseButtonDown(0))
        { startClick = Input.mousePosition; }
        else if (Input.GetMouseButtonUp(0))
        { startClick = -Vector3.one; }

        //caso o butoão seja pressionado anteriorment executa esta parte do código tb, 
        //em que cria o rectangulo da selection box, esta box possui as coordenadas do rato
